...
use silent_caption_core::protocol::{encode, Frame, MessageType, ProtocolError};
...
fn hello_reports_boot_change_and_invalidates_session() {
    ...
    assert_eq!(
        ...
        [MobileEvent::Hello { boot_id: 1, rebooted: false }]
    );
    boundary.start_session(42).unwrap();
    assert_eq!(
        boundary.accept_frame(&hello(2)).unwrap(),
        [MobileEvent::Hello { boot_id: 2, rebooted: true }]
    );
    let audio = frame(MessageType::AudioData, 1, 42, vec![1]);
    assert_eq!(boundary.accept_frame(&audio), Err(ProtocolError::StaleSession));
}
...
fn corrupt_crc_and_stale_session_are_rejected() {
    ...
    assert_eq!(boundary.accept_frame(&stale), Err(ProtocolError::StaleSession));

    let mut corrupt = frame(MessageType::AudioData, 1, 8, vec![1]);
    *corrupt.last_mut().unwrap() ^= 1;
    assert_eq!(boundary.accept_frame(&corrupt), Err(ProtocolError::Integrity));
}
...
fn gaps_duplicates_and_resets_are_explicit_events() {
    ...
    boundary.accept_frame(&frame(MessageType::AudioData, 10, 5, vec![1])).unwrap();
    let duplicate = boundary.accept_frame(&frame(MessageType::AudioData, 10, 5, vec![2])).unwrap();
    assert_eq!(duplicate[0], MobileEvent::SequenceDuplicate);
    let gap = boundary.accept_frame(&frame(MessageType::AudioData, 13, 5, vec![3])).unwrap();
    assert_eq!(gap[0], MobileEvent::SequenceGap { missing: 2 });
    let reset = boundary.accept_frame(&frame(MessageType::AudioData, 2, 5, vec![4])).unwrap();
